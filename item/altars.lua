...
-- gods.GOD_BRAGON   -  2 - Br�gon - God of fire
...
--Younger Gods
-- gods.GOD_NARGUN   -  6 - Narg�n - God of chaos
...
-- gods.GOD_MALACHIN - 11 - Malach�n - God of battle and hunting
...
local function canDevote(User, god)
    ...
    if not checkItems(User, gods.getItemsForDevotion(User, god)) then
        ...
        reason_de = reason_de .. "Du hast nicht die geforderten Gegenst�nde. "
    end
...
local function doDevote(User, god)
    ...
    if not candevote then
        ...
        common.InformNLS(User, "Du erf�llst nicht die Bedingungen, die an einen Anh�nger gestellt werden. " .. reason_de, "You do not satisfy the devotion criteria. " .. reason_en)
        return
    end
...
local function devotionDialog(User, god)
    ...
    elseif gods.isPriest(User) then
        explanation = explanation .. common.GetNLS(User,
            "Als Priester einer anderen Gottheit m�sstest du deiner Gottheit abschw�ren, um ein Priester " .. gods.getNameDe(god) .. "s zu werden. Du w�rdest ebenso deinen Status als Anh�nger verlieren.",
    ...
    elseif gods.isDevoted(User) then
        explanation = explanation .. common.GetNLS(User,
            "Als Anh�nger einer anderen Gottheit wirst du deinem Gott abschw�ren m�ssen, um dich " .. gods.getNameDe(god) .. " zu weihen.",
            "As devotee of another god, you'll have to abjure your god to devote yourself to " .. gods.getNameEn(god) .. "."
        )
    else -- noob
        explanation = explanation .. common.GetNLS(User, "Du kannst nur Anh�nger eines Gottes sein. Abschw�ren ver�rgert deine Gottheit.", "You can only devote yourself to one god at a time, and abjuration will make your god angry.")
    end
    explanation = explanation .. common.GetNLS(User, "\nSpende diese Gegenst�nde: " .. tellItems(User, gods.getItemsForDevotion(User, god)) .. "." , "\nYou will need to donate " .. tellItems(User, gods.getItemsForDevotion(User, god)) .. ".")

    local candevote, reason_de, reason_en = canDevote(User, god)
    if candevote then
        common.selectionDialogWrapper(User, common.GetNLS(User, "Anh�ngerschaft", "Devotion"), explanation, {
            { icon = 0, text = common.GetNLS(User, "Erkl�re dich zum Anh�nger von " .. gods.getNameDe(god), "Devote yourself to " .. gods.getNameEn(god)), func = doDevote, args = { User, god } },
        ...
        local dialog = MessageDialog(common.GetNLS(User, "Anh�ngerschaft", "Devotion"), explanation, --[[callback=]]function(d) end)
...
local function sacrificeExplanation(User, god)
    local explanation = common.GetNLS(User,
        "Um Items an deinen Gott zu opfern, platziere sie auf dem Boden, nahe des Altars. Falls deine Gabe dem Gott gef�llt, wird sie angenommen. Andernfalls kannst du sie zur�cknehmen und etwas andere, passenderes, ausprobieren.",
...
local function defile(User, god)
    common.TalkNLS(User, Character.say , "#me spuckt auf den Altar und verflucht den Namen von " .. gods.getNameDe(god), "#me spits on the altar and curses the name of " .. gods.getNameEn(god))
    local favour_penalty
    if gods.isPriest(User, god) then
        -- Apostate!
        gods.setNotPriest(User)
        favour_penalty = gods_common.DEFILE_BASE_INCREMENT * 5
    elseif gods.isDevoted(User, god) then
        favour_penalty = gods_common.DEFILE_BASE_INCREMENT * 2
    else
        favour_penalty = gods_common.DEFILE_BASE_INCREMENT
    end
    if gods_common.defileCooldownCounter:hasEnded(User) then
        gods.increaseFavour(User, god, -favour_penalty)
        gods_common.defileCooldownCounter:restart(User)
    else
        common.InformNLS(User, gods.getNameDe(god) .. " ist immer noch beleidigt durch Ihre letzte Schändung.", gods.getNameEn(god) " is still offended by your last defilement.")
...
local function ZeniaAltar(User, SourceItem)
    ...
    elseif User:getQuestProgress(502) == 8 and User:countItemAt("all", 2760) >= 2 and User:countItemAt("all", 3) >= 5 and User:countItemAt("all", 26) >= 5 and User:countItemAt("all", 73) >= 10 then -- Take raft items
        ...
        User:inform("[Quest status] Nach dem Gebet siehst du, wie die Materialien sich von selbst zu einem kleinen Floss zusammenf�gen. Du kletterst darauf und wirst zu einer kleinen Insel transportiert.", "[Quest status] After praying, you see the items magically�form a small raft.  You climb on board and are transported to a small island.")
    ...
    elseif User:getQuestProgress(502) == 8 then -- You dropped something you needed.
        User:inform("Du musst alle Einzelteile f�r Flo� und Gebet haben.", "You need all items for raft and prayer.")
    ...
    elseif  User:getQuestProgress(502) == 10 then -- You dropped something you needed.
        User:inform("Du hast nicht die richtigen Dinge f�r das Gebet.", "You lack the proper items for prayer.")
    end
end
...
function M.LookAtItem(User, Item)
    ...
    if not gods.GODS[god] then --undedicated altar
        ...
        if gods.isDevoted(User, god) then
            lookat.SetSpecialDescription(Item, "Der Anblick von " .. gods.getNameDe(god) .. "s Altar erf�llt dich in deiner Ergebenheit mit Stolz.", "Beholding the altar of " .. gods.getNameEn(god) .. " makes you feel proud of your devotion.")
        end
...
function M.UseItem(User, SourceItem, ltstate)
    ...
    if not gods.GODS[god] then --undedicated altar
        common.InformNLS(User, "Du ber�hrst den Altar, die Abwesenheit g�ttlichen Wirkens ist offensichtlich.", "You touch the altar, the absence of divine blessing is obvious.");
    else --dedicated altar
...
--            )else
        ...
        elseif not gods.isDevoted(User, god) then
            table.insert(dialogOptions,
